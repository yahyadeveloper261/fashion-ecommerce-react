...
export default function BestSellers({dark}) {
  const {search}=useContext(SearchContext);
  const [sortBy,setSortBy]=useState("")
  const [currentPage,setCurrentPage]=useState(1);
  const productsPerPages=8;
  const filterSearch=products.filter(item=>item.name.toLowerCase().includes(search.toLowerCase()))
  const bestSellers = filterSearch.filter((item) => item.bestSeller);

  
const sortedBy=sortBy===""?bestSellers:[...bestSellers].sort((a,b)=>sortBy==="low"?a.price-b.price:
  sortBy==="high"?b.price-a.price:
sortBy==="a-z"?a.name.localeCompare(b.name):b.name.localeCompare(a.name))
...
{bestSellers.length === 0 && filterSearch.length!==0 && (
  <h2 className="text-xl font-semibold text-gray-500 text-center mt-10">
  No best sellers products available at the moment.
  </h2>
)}
