...
export default function Brands({dark}) {
...
const productsPerPages=8;
  ...
  const nikeProducts =filterSearch.filter(
    (item) => item.brand === "Nike"
  );
  
  const sortedBy=sortBy===""?nikeProducts:[...nikeProducts].sort((a,b)=>sortBy==="low"?a.price-b.price:
...
{nikeProducts.length === 0 && filterSearch.length!==0 && (
  <h2 className="text-xl font-semibold text-gray-500 text-center mt-10">
  No brads  available at the moment.
  </h2>
)}
