...
export default function Kids({ dark }) {
  const { subCategory } = useParams();
  const navigate = useNavigate();
  const { search } = useContext(SearchContext);
  const [sortBy, setSortBy] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const totalPerPages = 8;

  const filterSearch = products.filter((item) =>
    item.name.toLowerCase().includes(search.toLowerCase()),
  );

  const kidProducts = filterSearch.filter((item) => item.category === "Kids");
  ...
  const totalPages = Math.ceil(sortedBy.length / totalPerPages);
  const startIndex = (currentPage - 1) * totalPerPages;
  const endIndex = startIndex + totalPerPages;
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        >
          ...
          >
            ...
            {/* T-Shirts */}
            <button
              onClick={() => navigate("/kids/t-shirts")}
              className={`shrink-0 px-4 py-2 rounded-full ${
            ...
            {/* Trousers */}
            <button
              onClick={() => navigate("/kids/trousers")}
              className={`shrink-0 px-4 py-2 rounded-full ${
        ...
        {filterProducts.length === 0 && filterSearch.length !== 0 && (
          <div className="col-span-full flex justify-center items-center py-16 px-4">
            <div className="text-center animate-pulse">
              ...
              >
                No {subCategory || "Kids Products"} Available
              </h2>
